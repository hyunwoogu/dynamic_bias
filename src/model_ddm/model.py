""" Drift-diffusion models of decision-consistent bias
    Author: Hyunwoo Gu
"""
import time
import numpy as np
...
class DynamicBiasModel:
    ...
    def transform(self, params, direction='fwd'):
        # [0,1] normalization to help optimization
        lb = self.bounds.lb
        ub = self.bounds.ub
    ...
    def forward(self, params, data):
        """forward model
            for full model, parameters are: [w_K, w_E, w_P, w_D, w_r, w_b, lam, s]
            for reduced model, parameters are: [w_E, w_P, w_D, w_r, w_b, lam, s]
        """
        # parameters
        if self.model == 'full':    
        ...
        for i_delay, delay in enumerate(self.delay_condition):
            ...
            for r in self.relative_reference:
                ...
                P[0][:, idx] = expm(Lp) @ expm(L2[i_delay]) @ expm(self.Hdrift*(-w_b*(np.abs(r)<self.near_cutoff)-r*w_r)) @ _P_cw[:,idx]
                P[1][:, idx] = expm(Lp) @ expm(L2[i_delay]) @ expm(self.Hdrift*( w_b*(np.abs(r)<self.near_cutoff)-r*w_r)) @ _P_ccw[:,idx]
